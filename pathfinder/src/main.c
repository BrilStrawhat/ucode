...
static void arr_malloc(char ***adj_matrix, int island_count) {
    printf("%lu\n", island_count * sizeof(char *));
    for (int i = island_count; i >= 0; i--) {
        adj_matrix[i] = (char **)malloc(island_count * island_count * sizeof(char *));
    }
    for (int i = island_count; i >= 0; i--) {
...
// (char ***adj_matrix, char ***result_matrix, int island_count) {
    ...
    // while (step++ <= island_count) {
        ...
        // else
            // copy_static_col_row((current == 1)? result_matrix : adj_matrix , adj_matrix, step, island_count);
        // for (int i = 1; i <= island_count; i++) {
...
int main(int argc, char **argv) {
    ...
    char ***adj_matrix = (char***)malloc((island_count + 1) * (island_count + 1) * sizeof(char **)); // fill last COL and ROW '\0'
    char ***result_matrix = (char***)malloc((island_count + 1) * (island_count + 1) * sizeof(char **)); // fill last COL and ROW '\0'
    // char *island_i;
    // char *island_j;
    ...
    // str **adj_matrix = (str **)malloc(sizeof(str *) * (island_count + 1));
    // for (int i = 0; i <= island_count; i++)
        // adj_matrix[i] = (str *)malloc(sizeof(str) * (island_count + 1));
    // adj_matrix[0][0].str = mx_strdup("str\n");
    // mx_strdel(&adj_matrix[0][0].str);
    // for (int i = 0; i <= island_count; i++)
        // free(adj_matrix[i]);
    // free(adj_matrix);
    // for (int i = 0; i <= island_count; i++)
        // for (int j = 0; j <= island_count; j++)
            // adj_matrix[i][j].str = malloc(sizeof(str));

    // print_matrix(adj_matrix, island_count);
    // print_matrix(result_matrix, island_count);
    // mx_del_matrix(adj_matrix, island_count + 1);
    // mx_del_matrix(result_matrix, island_count + 1);
